...
describe("Auth routes", () => {
  describe("POST /api/auth/register", () => {
    ...
    it("should return 201 on successful register", async () => {
      jest
        .spyOn(authServices, "registerUserService")
        .mockResolvedValue({
          id: 1,
          username: "testuser",
    ...
    it("should return 400 if username/email taken", async () => {
      jest
        .spyOn(authServices, "registerUserService")
        .mockRejectedValue(new Error("USERNAME_OR_EMAIL_TAKEN"));
  ...
  describe("POST /api/auth/login", () => {
    ...
    it("should return 200 with tokens and user", async () => {
      jest
        .spyOn(authServices, "loginUserService")
    ...
    it("should return 500 on service failure", async () => {
      jest.spyOn(authServices, "loginUserService").mockRejectedValue(new Error("DB_ERR"));
      const res = await request(app)
        .post("/api/auth/login")
        .send({ username: "testuser", password: "password123" });
      expect(res.status).toBe(500);
      expect(res.body.error).toBe("Erreur serveur lors de la connexion");
    });
  });
